      ...
      <DialogContent className="sm:max-w-[1200px] h-[900px] justify-center !rounded-3xl overflow-hidden">
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="grid grid-cols-4 items-center gap-44">
            ...
            <Input onChange={handleChange} id="numberOfEmployees" type="number" className="col-span-3" />
          </div>


          <div className='flex'>
            {
                error?.some((err: any) => err.includes("Total Employees are required")) ? (
                  error.map((err: any, index: number) => {
                    if (
            ...
            <Input onChange={handleChange} id="since" type="number" className="col-span-3" />
          </div>

          <div className='flex'>
            {
                error?.some((err: any) => err.includes("Establishment Year is required")) ? (
                  error.map((err: any, index: number) => {
                    if (
                  ...
                  error.map((err: any, index: number) => {
                    if (
                      err.includes("Establishment Year is required") ||
                      err.includes("Establishment Year must be valid(after 1990)") ||
                      err.includes("Establishment Year must be valid(before 2025)") 
                    ) {
                      return (
                        <div key={index} className="text-start">
                          <span className="text-red-400 text-sm">{err}</span>
