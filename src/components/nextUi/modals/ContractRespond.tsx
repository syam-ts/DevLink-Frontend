  ...
  const submitResponse = async () => {
    try {
      ...
      const { data } = await axios.post(
        `http://localhost:3000/user/contact/response`,
        body
      );
  ...
  return (
    <>
      <div className="flex flex-wrap gap-3 bg-white">
        ...
        <Button
          className="bg-slate-800 rounded-md text-white font-bold"
          key={size}
        ...
        <ModalContent>
          {() => (
            <>
              ...
              <ModalBody>
                <p>
                  <label className="font-bold text-lg pb-2">
                    {" "}
                    Enter the description about the job{" "}
                  </label>
                  <textarea
                    onChange={handleChange}
                    name="description"
                    className="h-44 w-full border-black border"
                ...
                <div className="grid font-bold text-lg">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    type="number"
                    className="w-28 h-10 border border-black"
                    placeholder="100%"
                  />
                </div>
              </ModalBody>
