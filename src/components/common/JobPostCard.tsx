...
export const JobPostCard = ({
  ...
  {Object.entries(jobs).map((job: JobsParams[], index: number) => (
    <div
      key={index}
      className="w-full transform relative transition duration-500 hover:scale-105 border-gray-100 shadow-lg hover:shadow-xl rounded-xl border mx-auto my-6 p-4 md:p-6 arsenal-sc-regular"
    >
      <div className="flex flex-col lg:flex-row justify-between gap-4 md:gap-6">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm md:text-base mt-2 text-gray-700">{job[1]?.description}</p>
        ...
        <div className="mt-4 lg:mt-0 flex flex-col lg:items-end">
          <div className="grid grid-cols-3 lg:grid-cols-1 gap-2 text-sm md:text-base">
            ...
            <span className="lg:text-right">{job[1]?.estimateTimeinHours}/hr</span>
          </div>
          <span className="text-sm text-green-400 underline mt-1">
            {job[1]?.projectType}
          </span>
          <button
            className="rounded-md transform relative transition duration-500 hover:scale-110 bg-black px-4 py-2 md:px-6 border border-transparent text-center text-sm md:text-base text-white shadow-md hover:shadow-lg mt-3 w-full lg:w-auto"
            type="button"
          >
            <Link
