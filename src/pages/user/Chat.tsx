...
export const Chatbot = () => {
  ...
  const sendMessage = async (e: any) => {
    ...
    try {
      ...
      const { data } = await apiInstance.axiosInstanceUser.post('http://localhost:3000/user/chatbot', {
        userInput
      });


      setChatHistory([...chatHistory, { type: "user", message: userInput }, { type: "bot", message: data?.queryResult }])
  ...
  return (
    <div className='mx-auto py-96 p-3'>
...

      {
        open && (
          <div
            className="fixed shadow-2xl bottom-[calc(4rem+1.5rem)] right-0 mr-4 bg-white p-6 rounded-lg w-[450px] h-[650px]">

            <div className="flex flex-col space-y-1.5 pb-6">
              <h2 className="font-semibold text-lg tracking-tight">Devlink Chatbot</h2>
            </div>
            ...
            {chatHistory.map((chat) => (
              <div className="pr-4"  >
                ...
                <div className="flex gap-3 my-4 text-gray-600 text-sm flex-1"><span
                  ...
                  {chat.type === 'user' && (<p className="leading-relaxed"><span className="block font-bold text-gray-700">
                    Bot
                  </span>
                    {chat?.message}
                  </p>)}

                  {chat.type === 'bot' && (<p className="leading-relaxed"><span className="block font-bold text-gray-700">
                    Bot
                  </span>
                    {chat?.message}
                  </p>)}
            ...
            <div className="flex w-[400px] items-center pt-0 bottom-0 fixed mb-44">
              <form className="flex  items-center justify-center w-full space-x-2">
                <input onChange={(e: any) => handleUserInput(e)}
                  ...
                  placeholder="Ask to bot" value={userInput} />
