    ...
    (async () => {
      try {
        const { data } = await axios.get(
          `http://localhost:3000/admin/viewSingleContract/${contractId}`
        );
        setContract(data?.contract);
        console.log("The repsonse: ", data);
      } catch (err: any) {
              ...
              <section>
                <div className="mt-8 sm:justify-center arsenal-sc-regular">
                  ...
                  <div className="font-semibold">
                    ...
                    <hr className="bg-black " />
                    <div>
                      <ul className="text-md grid gap-3">
                        <li>
                          Title :<span className='font-light'> {contract?.jobPostData?.title}</span>
                        </li>
                        <li>
                          Description :
                          <span className='font-light'> {contract?.jobPostData?.description}</span>
                        </li>
                        <li>
                          Expert Level :
                          <span className='font-light'> {contract?.jobPostData?.expertLevel}</span>
                        </li>
                        <li>
                          Project Type :
                          <span className='font-light'> {contract?.jobPostData?.projectType}</span>
                        </li>
                        <li>
                          Status:<span className='font-light'> {contract?.status}</span>
                        </li>
                        <li>
                          Payment:<span className='font-light'> {contract?.amount}rs</span>
                        </li>
                        <li>
                          Payment Type:
                          <span className='font-light'> {contract?.jobPostData?.paymentType}</span>
                        </li>
                        <li>
                          Deadline:<span className='font-light'> {contract?.deadline}</span>
                        </li>
                      </ul>
                    </div>
                  </div>
